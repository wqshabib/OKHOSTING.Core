...
namespace OKHOSTING.Core
{
	...
	{
		/// <summary>
		/// Here we store the actual type (platform specific) that we must invoke when creating an object
		/// </summary>
        ...
        /// Use this for platform specific features like storage, streaming, settings or whatever you need
        /// </remarks>
        public static T Create<T>(IEnumerable<string> possiblePlatforms)
        {
            ...
            return (T) Activator.CreateInstance(PlatformSpecificTypes[baseType]);
        }
	}
}